    ...
    <svg className="h-6 w-6 md:h-8 md:w-8 lg:h-10 lg:w-10 text-cyan-400 "
      ...
      <figure className="flex items-center gap-2 xl:gap-5 md:gap-5">
        ...
        {CameraStreams.length > 0 && (
          <>
          <button onClick={clearCameraStreams} className="cursor-pointer outline-1 outline-cyan-700 w-40 h-10 rounded-lg text-gray-100 bg-gray-900 hidden md:block md:w-35 md:h-8 md:text-[14px]"><i className="fa-solid fa-trash"></i> clear all camera</button>
        <button onClick={clearCameraStreams} className="relative group cursor-pointer text-gray-100  md:hidden xl:hidden w-8 h-8 p-2 bg-gray-900 text-[12px] rounded-full"><i className="fa-solid fa-trash"></i><span className="hidden group-hover:block absolute bottom-10">clear all cameras</span></button> 
        </>)}
